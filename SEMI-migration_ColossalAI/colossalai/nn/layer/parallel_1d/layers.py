...
from ._operation import linear_with_async_comm, rowcut_linear_with_async_comm
from ._utils import (
...
class Linear1D_Col(ParallelLayer):
    ...
    def __init__(self,
        ...
        self.iterations_per_epoch = gpc.get_iterations_per_epoch()
        self.iteraion_num = 0
        self.eval_flag = 1
        weight_num = self.weight.shape[1]
        self.priority_list = torch.arange(weight_num, device=device)
        self.compute_list = torch.arange(weight_num, device=device)
        self.migration_list = torch.arange(0, device=device)
        self.last_weight = self.weight.clone().detach()
        self.start_process = time.time()
        #logger = get_dist_logger()
        #logger.info(f'g_shape: {self.weight.shape}', ranks=[1])
        self.pre_weight_variation = torch.ones(weight_num, device=device)
    ...
    def forward(self, input_: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        output_parallel = linear_with_async_comm(input_parallel, self.weight, bias, ParallelMode.PARALLEL_1D, True, self.priority_list, self.cur_epoch, self.eval_flag, self.migration_list, self.compute_list)
        if self.gather_output:
            # All-gather across the partitions.
            output = gather_forward_split_backward(output_parallel, ParallelMode.PARALLEL_1D, dim=-1)
        else:
        ...
        #logger = get_dist_logger()
        #logger.info(f'grad_shape: {output.shape}', ranks=[0])
        
        #zx:优先级处理
        self.iteraion_num = self.iteraion_num + 1
        #if self.iteraion_num > 1563:
        if self.iteraion_num > 782:
            self.eval_flag = 0
        
        if self.iteraion_num%self.iterations_per_epoch == 0:
            #zx:记录时间
            #start_process = time.time()
            device = torch.cuda.current_device()
            self.end_process = time.time()
            epoch_time =self.end_process - self.start_process
            #zx:创建优先级列表
            self.cur_epoch = self.cur_epoch + 1
            self.iteraion_num = 0
            weight_num = self.weight.shape[1]
            cut_num = 0
            #cut_num = int(weight_num/2)
            res_num = weight_num-cut_num

            if device==0:
                migration_num = int(res_num/32)*4*7
            else:
                migration_num = 0
            #migration_num = int(res_num/14)*7
            compute_num = res_num - migration_num

            weight_variation = torch.sub(self.weight,self.last_weight)
            weight_variation = torch.abs(weight_variation)
            weight_variation = torch.sum(weight_variation,0)

            descend_num = torch.index_select(self.pre_weight_variation, dim=0, index=self.priority_list)
            add_num = torch.index_select(weight_variation, dim=0, index=self.priority_list)
            res_weight_variation = self.pre_weight_variation.clone()
            res_weight_variation.index_add_(0, self.priority_list, descend_num, alpha=-1)
            res_weight_variation.index_add_(0, self.priority_list, add_num, alpha=1)
            self.pre_weight_variation = res_weight_variation
            
            values, indice1 = torch.topk(res_weight_variation, k=res_num, largest=True, sorted=True)

            cmp_list = torch.split(indice1, [compute_num, migration_num])[0]
            mig_list = torch.split(indice1, [compute_num, migration_num])[1] 
            
            sorted, indice2 = torch.sort(indice1)
            self.priority_list = sorted.to(device) 
            sorted2, indice3 = torch.sort(mig_list)
            self.migration_list = sorted2.to(device) 
            sorted3, indice4 = torch.sort(cmp_list)
            self.compute_list = sorted3.to(device) 
            '''
            random_list = random.sample(range(0,weight_num),res_num)
            indice1 = torch.tensor(random_list)
            sorted, indice2 = torch.sort(indice1)
            self.priority_list = sorted.to(device)
            '''     
            '''
            weight_variation_list = weight_variation.tolist()
            max_indice = 0
            max_sum = 0
            for i in range(0,len(weight_variation_list)-res_num):
                tmp_sum = 0
                for j in range(i, i + res_num):
                    tmp_sum = tmp_sum + weight_variation_list[j]
                if tmp_sum > max_sum:
                    max_indice = i
                    max_sum = tmp_sum           
            #max_indice = random.randint(0, res_num-1)
            self.max_indice = max_indice
            self.priority_list = torch.arange(self.max_indice, self.max_indice + res_num, device=device)
            '''
            self.last_weight = self.weight.clone().detach()
            #end_process = time.time()
            self.start_process = time.time()
            #per_epoch_time = torch.tensor(epoch_time, device=device)
            #handle = dist.all_reduce(per_epoch_time, group=gpc.get_group(ParallelMode.PARALLEL_1D), async_op=True)
            #handle.wait()
            #logger = get_dist_logger()
            #logger.info(f'priority_list: {self.priority_list}, device: {device}', ranks=[0])
            #logger.info(f'priority_list_shape: {self.priority_list.shape}, device: {device}', ranks=[0])
            #logger.info(f'times: {epoch_time}, device: {device}', ranks=[0,1])
            #logger.info(f'times: {per_epoch_time}, device: {device}', ranks=[0,1])
...
class Linear1D_Row(ParallelLayer):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        if self.stream_chunk_num > 1:
            ...
            #output_parallel = F.linear(input_, self.weight)
            #output_parallel = linear_with_async_comm(input_, self.weight, None, ParallelMode.PARALLEL_1D, False, self.cur_epoch)
            output_parallel = rowcut_linear_with_async_comm(input_, self.weight, None, ParallelMode.PARALLEL_1D, False, self.priority_list, self.cur_epoch, self.eval_flag, self.migration_list, self.compute_list)
            output = reduce_input(output_parallel, ParallelMode.PARALLEL_1D)
