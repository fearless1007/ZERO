...
def main():
    ...
    for p in model.parameters():
        total_numel += p.numel()
    logger.info(f"number of parameters: {total_numel}")
    ...
    root = '/home/haida_huanglei/yky/tmp/Imagenet-1k/'
    ...
    # train_dataloader = DummyDataloader(length=3, batch_size=gpc.config.BATCH_SIZE)

    # test_dataloader = DummyDataloader(length=30, batch_size=gpc.config.BATCH_SIZE)
    ...
    for epoch in range(gpc.config.NUM_EPOCHS):
        ...
        # compute the number of correct prediction
           ...
           # print(pred,label)
            ...
            total += img.size(0)
        logger.info(
            f"Epoch {epoch} -  test loss: {test_loss:.5}, acc: {correct / total:.5}, lr: {lr_scheduler.get_last_lr()[0]:.5g}", ranks=[0])    
        #save_checkpoint(f'test_check_model{epoch}.pt',epoch,model,optimizer,lr_scheduler)
    gpc.destroy()
